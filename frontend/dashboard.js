document.addEventListener('DOMContentLoaded', async () => {
    const API_URL = (window.apiBase || '') + '/api'; // This script is for authority/dashboard.html
    ...
    async function exportReports(format) { // This function is also in admin/admin.js, should be consolidated or removed from here
        ...
        if (!window.auth.checkUserRole(['admin', 'autoridad'])) return; // Use window.auth.checkUserRole()
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            if (typeof window.auth.checkUserRole !== 'function' || !window.auth.checkUserRole(['admin', 'autoridad'])) return; // Use window.auth.checkUserRole()
            openExportModal();
        });
    }
    ...
    if (exportPdfBtn) exportPdfBtn.addEventListener('click', async () => { await exportReports('pdf'); closeExport(); }); // Use window.auth.getAuthToken()
    if (exportExcelBtn) exportExcelBtn.addEventListener('click', async () => { await exportReports('excel'); closeExport(); }); // Use window.auth.getAuthToken()
            ...
            try {
                ...
                try {
                    ...
                    if (parts.length >= 2) {
                        ...
                        userRole = payload.user && payload.user.rol ? payload.user.rol : null;
                    }
                } catch (e) { /* ignore */ }
                ...
                if (response.ok) {
                    // Filter reports for the logged-in authority
                    const authorityReports = data.allReports.filter(r => r.asignado_id === userId);

                    totalReportsElement.textContent = authorityReports.length;
                    pendingReportsElement.textContent = authorityReports.filter(r => r.estado === 'Pendiente').length;
                    inProcessReportsElement.textContent = authorityReports.filter(r => r.estado === 'En proceso').length;
                    resolvedReportsElement.textContent = authorityReports.filter(r => r.estado === 'Resuelto').length;

                    // Render recent reports (filtered)
                    recentReportsTableBody.innerHTML = '';
                    authorityReports.slice(0, 5).forEach(report => { // Show up to 5 recent reports
                        const row = `
                    ...
                    // Render charts (assuming renderCharts function is available globally or defined here)
                    // For authority, charts should also reflect only their assigned reports.
                    // This might require backend support for authority-specific chart data or client-side aggregation.
                    // For now, we'll pass the filtered reports to renderCharts.
                    if (typeof renderCharts === 'function') {
                        const reportsByCategory = authorityReports.reduce((acc, r) => {
                            acc[r.categoria] = (acc[r.categoria] || 0) + 1;
                            return acc;
                        }, {});
                        const reportsByStatus = authorityReports.reduce((acc, r) => {
                            acc[r.estado] = (acc[r.estado] || 0) + 1;
                            return acc;
                        }, {});
                        const reportsByDistrict = authorityReports.reduce((acc, r) => {
                            acc[r.distrito] = (acc[r.distrito] || 0) + 1;
                            return acc;
                        }, {});

                        renderCharts(
                            Object.entries(reportsByCategory).map(([categoria, count]) => ({ categoria, count })),
                            Object.entries(reportsByStatus).map(([estado, count]) => ({ estado, count })),
                            Object.entries(reportsByDistrict).map(([distrito, count]) => ({ distrito, count }))
                        );
                    } else {
