// autoridad.js
// Lógica para tareas.html (gestión de tareas de la autoridad)
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!token) { // Check token after ensuring window.auth is available
        window.location.href = 'index.html';
        return;
    }
    ...
    try {
        ...
        if (parts.length >= 2) {
            ...
            userRole = window.auth.getRoleFromToken(token); // Use window.auth.getRoleFromToken
    ...
    async function fetchAndRenderReports() {
        let url = (window.apiBase || '') + '/api/dashboard/mis-tareas';
        try {
            ...
            if (searchInput && searchInput.value) {
                ...
                reports = reports.filter(r =>
                    ...
                    (r.direccion && r.direccion.toLowerCase().includes(search))
                );
            }
            if (statusFilter && statusFilter.value) {
                reports = reports.filter(r => r.estado === statusFilter.value);
            }
            if (categoryFilter && categoryFilter.value) {
                reports = reports.filter(r => r.categoria === categoryFilter.value);
            }
            if (districtFilter && districtFilter.value) {
                reports = reports.filter(r => r.distrito === districtFilter.value);
            }
            renderReportTable(reports);
        } catch (error) {
            alert('Error de conexión al obtener las tareas.');
        }
    }
