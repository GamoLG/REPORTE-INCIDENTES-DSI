document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (document.getElementById('adminDashboard') && window.auth.checkUserRole(['admin'])) {
        // The previous line already checks for 'admin'. If it's an authority, it shouldn't be here.
        // This redundant check is removed.
        ...
        const loadDashboardData = async () => {
            try {
                ...
                let recentReports = data.recentReports; // Admin dashboard shows all recent reports
    ...
    if (document.getElementById('reportManagementTable') && (window.location.pathname.includes('admin/report-management.html') || window.location.pathname.includes('authority/tasks.html'))) {
        if (!window.auth.checkUserRole(['admin', 'autoridad'])) return;
        ...
        async function fetchAndRenderReports() {
            ...
            try {
                ...
                const userRole = window.auth.getRoleFromToken(token); // Use window.auth.getRoleFromToken()
                let userId = null;
                if (userRole === 'autoridad') {
                    // Obtener el id del usuario desde el token
                    try {
                        const parts = token.split('.'); // Keep this as userId is not in window.auth
                        if (parts.length >= 2) {
    ...
    if (document.getElementById('reportDetail') && (window.location.pathname.includes('admin/report-detail.html') || window.location.pathname.includes('authority/task-detail.html'))) {
        if (!window.auth.checkUserRole(['admin', 'autoridad'])) return;

        const urlParams = new URLSearchParams(window.location.search);
        const reportId = urlParams.get('id');
        if (!reportId) { // Update redirect path
        ...
        async function fetchReportDetail() {
            try {
                ...
                if (response.ok) {
                    currentReport = data;
                    renderReportDetail(); // Call render function
                } else {
        ...
        function renderReportDetail() {
            if (!currentReport) return; // Ensure currentReport is loaded
    ...
    if (document.getElementById('profileForm') && window.location.pathname.includes('admin/profile.html')) {
        if (!window.auth.checkUserRole(['admin', 'autoridad'])) return;
        // Aquí irá la lógica para gestionar el perfil del admin/autoridad
    }

    // Lógica para admin-map.html (se implementará en una tarea posterior)
    if (document.getElementById('mapid') && window.location.pathname.includes('admin/map.html')) {
        if (!window.auth.checkUserRole(['admin', 'autoridad'])) return;
        // Aquí irá la lógica para el mapa de incidencias
    }
});